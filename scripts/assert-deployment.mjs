...
assert(
  contract.preview.analyticsEnabled === false &&
    ...
    contract.preview.features.emeraldVariation === true &&
...
assert(
  contract.production.analyticsEnabled === true &&
    ...
    contract.production.features.emeraldVariation === false &&
...
for (const [name, artifact] of [
  ...
  assert(
    artifact.features.emeraldVariation ===
      contract[expectedEnvironment].features.emeraldVariation,
    `${name} artifact emerald-variation flag does not match the source contract.`,
  );
  assert(
...
if (expectedEnvironment === "preview") {
  ...
  assert(
    clientArtifact.analyticsEnabled === true &&
      ...
      clientArtifact.features.emeraldVariation === false &&
...
assert(
  wrangler.vars.SL_DEPLOY_ENV === "production" &&
    wrangler.vars.SL_FEATURE_EMERALD_VARIATION === "false",
...
for (const [key, value] of Object.entries({
  ...
  SL_FEATURE_EMERALD_VARIATION: String(expected.features.emeraldVariation),
