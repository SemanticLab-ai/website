import { X, Mail, Sparkles } from "lucide-react";
import { Button } from "~/components/ui/button";
import { useState } from "react";

interface SignUpModalProps {
  open: boolean;
  onClose: () => void;
}

export function SignUpModal({ open, onClose }: SignUpModalProps) {
  const [email, setEmail] = useState("");

  if (!open) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      {/* Backdrop */}
      <div className="absolute inset-0 bg-charcoal/60 backdrop-blur-sm" onClick={onClose} />

      {/* Modal */}
      <div className="relative bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 text-slate hover:text-charcoal rounded-lg transition-colors"
        >
          <X className="w-5 h-5" />
        </button>

        <div className="text-center mb-6">
          <div className="mx-auto w-14 h-14 rounded-2xl bg-gradient-to-br from-vector-rose to-vector-rose-dark flex items-center justify-center mb-4 shadow-lg">
            <Sparkles className="w-7 h-7 text-white" />
          </div>
          <h2 className="text-2xl font-display font-bold text-charcoal mb-2">
            You've used your free images!
          </h2>
          <p className="text-slate">
            Create a free account to continue, or upgrade for batch processing and more.
          </p>
        </div>

        {/* Email input */}
        <div className="space-y-4 mb-6">
          <input
            type="email"
            placeholder="Enter your email"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            className="w-full px-4 py-3 rounded-xl border border-slate/20 text-charcoal placeholder:text-slate/50 focus:outline-none focus:border-vector-rose/50 focus:ring-2 focus:ring-vector-rose/10"
          />
          <Button
            size="lg"
            className="w-full h-12 rounded-full bg-gradient-to-r from-vector-rose to-vector-rose-dark text-white shadow-lg font-semibold"
            onClick={() => {/* Non-functional demo */}}
          >
            <Mail className="w-4 h-4 mr-2" />
            Create Free Account
          </Button>
        </div>

        {/* Divider */}
        <div className="flex items-center gap-4 mb-6">
          <div className="flex-1 h-px bg-slate/10" />
          <span className="text-xs text-slate">or</span>
          <div className="flex-1 h-px bg-slate/10" />
        </div>

        {/* Social buttons */}
        <div className="space-y-3">
          <button className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-pearl rounded-xl text-sm font-medium text-charcoal hover:bg-slate/10 transition-colors">
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4" />
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
            </svg>
            Continue with Google
          </button>
        </div>

        <p className="text-xs text-slate text-center mt-6">
          Free account includes 10 images/month. No credit card required.
        </p>
      </div>
    </div>
  );
}
