...
void main() {
  ...
  float angle = u_time*.018 + sin(u_time*.16 + depth*.5)*.10;
  ...
  v_light *= 1.-u_motion*.12 + sin(u_time*.8+seed*60.)*u_motion*.12;
...
export function createGalaxyRenderer(canvas: HTMLCanvasElement) {
  ...
  for (let i=0; i<count; i++) {
    const radius = Math.pow(random(), .70)*.53;
    const arm = i%3 * Math.PI*2/3;
    const theta = arm + radius*10.5 + (random()-.5)*(.6+radius*.8);
    const spread = (random()-.5)*.055;
    data.set([Math.cos(theta)*radius+spread, Math.sin(theta)*radius+spread, random(), random()], i*4);
  }
